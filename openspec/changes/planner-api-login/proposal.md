...
| 12  | Scraper contract unchanged; end-to-end run passes               | **Deviation recorded:** `getStatus()` is now `async` (`design.md` § AC-9). `planner-http.client.ts` is unmodified and never called it; `getValidSession`/`getValidToken`/`refresh` keep their signatures. End-to-end run is runbook step 7 — **not yet executed**                                                                                                                                                                                                                                                                                                                                         |
