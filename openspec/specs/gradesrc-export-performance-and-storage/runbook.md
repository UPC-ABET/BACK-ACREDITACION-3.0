...
| 2   | Deploy this change (migration first, per the deploy prerequisite above).                                                                                                                                                                                                                                                                                                                                                                                             | `GET .../gradesRc/status?period=202610` still reports whatever status step 1 left it in (`completed`, most likely) — the migration backfills any pre-existing `core.scraping_export_gradesrc_rows` batch into `rows_data` before dropping the table (see the migration's `up()`), specifically so a row that was already `completed` pre-deploy does not go on reporting `completed` while `download` silently returns null. `GET .../gradesRc/download?lang=es` for 202610 should therefore still succeed at this point, serving step 1's pre-fix content, before step 3 even runs.              |
