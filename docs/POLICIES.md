# ABET Backend — POLICIES

> **Mandatory rules.** Everything here is a constraint you can violate, so everything here
> is checked in review. If you believe a rule is wrong, that is a conversation with the
> team — not a decision to make inside a pull request.
>
> Orientation (stack, structure, domain vocabulary, environment) lives in
> [CONTEXT.md](./CONTEXT.md). Architectural decisions and their reasoning live in
> [adr/](./adr/).
...
The database configuration itself is described in [CONTEXT.md § Database](./CONTEXT.md#database).
...
`openapi.json` is generated from the Swagger decorators and **committed** to this repository.
It is the frontend's source of truth for this API.

- **Regenerate and commit it in the same pull request** as any change to a route, a DTO, or
  a response shape: `pnpm openapi:export`.
- A spec that lags the endpoints is worse than no spec, because the frontend trusts it and
  has no way to detect the lag.
- Swagger decorators are therefore **load-bearing, not documentation**. An endpoint with no
  typed `@ApiResponse`, or a DTO whose decorators disagree with its TypeScript type,
  produces a spec that lies. Treat that as a defect.
- Scope headers must appear in the spec — apply the Swagger header decorator alongside the
  value decorator, or the frontend cannot see that the endpoint requires them.
...
**Scope must survive every asynchronous hop.** It is request-derived, so anything that carries
work into a cache key, a background job, or a shared singleton and drops the scope will serve
one school's data to another — silently, and looking entirely correct. Treat every new cache
key and every new async path as a scope-leak candidate.
...
**Watch for the fixture trap.** A factory default that makes the condition under test a no-op
produces a test that passes for the wrong reason — worse than no test, and the most common
false positive on this codebase. Confirm a new test fails before the change that makes it pass.

## Git & Commits

- Base branch for every pull request is **`develop`**. Promotion is
  `develop → staging → production`, fast-forward only.
- Branches are `<type>/<kebab-slug>`, where the slug matches the change folder:
  `feat/bulk-edit-rubric-weights`.
- Commit messages are **Conventional Commits, subject only** — a single line, no body, no
  trailers, no `Co-Authored-By`.
- **Never use `--no-verify`.** If a hook fails, fix the underlying problem.
...
- **Don't change a route, DTO or response shape without regenerating `openapi.json`** in the same PR.
