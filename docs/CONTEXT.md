...
| `core`          | types, type groups, parameters, email templates, notification logs, scraper credentials (password encrypted — see [ADR-001](./adr/ADR-001-external-system-credentials-encrypted-in-database.md)), persisted scraping export generation state (`scraping_export_runs`, language-neutral and keyed on `(exportType, period)`, plus `scraping_export_gradesrc_rows` for the materialized gradesRc merge — see [ADR-003](./adr/ADR-003-language-neutral-scraping-export-generation.md), which supersedes [ADR-002](./adr/ADR-002-persisted-pollable-scraping-export-generation.md)'s per-language file storage) |
...
- **A gradesRc generation's previous batch of `core.scraping_export_gradesrc_rows` is deleted
  only once the new batch has finished inserting and the parent `scraping_export_runs` row has
  flipped to `completed` — never before.** Mirrors the Banner/Planner "latest completed run only"
  rule above: rows are tagged with the `generatedAt` timestamp of the generation that produced
  them, so a `download` mid-regenerate keeps serving the previous batch instead of a gap.
  Enforced in `ScrapingExportGenerationService.runGradesRcGeneration()`. See
  [ADR-003](./adr/ADR-003-language-neutral-scraping-export-generation.md).
