...
| `core`          | types, type groups, parameters, email templates, notification logs, scraper credentials (password encrypted — see [ADR-001](./adr/ADR-001-external-system-credentials-encrypted-in-database.md)), persisted scraping export generation state (`scraping_export_runs`, language-neutral, keyed on `(exportType, period)`, and holding all five export types' rows in one `rows_data` jsonb column — see [ADR-003](./adr/ADR-003-language-neutral-scraping-export-generation.md), superseded on the `gradesRc` storage point by [ADR-004](./adr/ADR-004-gradesrc-rows-in-shared-jsonb-storage.md), which supersedes [ADR-002](./adr/ADR-002-persisted-pollable-scraping-export-generation.md)'s per-language file storage) |
...
- **`gradesRc` generation writes its rows through the same single-column `rows_data` update the
  other four export types use — there is no separate batch/retention step.** A single-row jsonb
  write is already atomic under Postgres's MVCC, so a `download` racing a `regenerate` sees either
  the whole old array or the whole new one, never a torn mix, without needing the
  `generatedAt`-tagged-batch dance a dedicated child table would require. See
