...
import { FindingValidation } from './findings.validation';
...
describe('FindingValidation', () => {
	...
	describe('validateDelete', () => {
		it('passes when entity exists', async () => {
			mockRepo.findOneById.mockResolvedValue({ id: 1 });
			await expect(FindingValidation.validateDelete(mockRepo as any, 1)).resolves.toBeUndefined();
		});
