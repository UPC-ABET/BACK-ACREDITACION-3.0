...
export type DispatchReason = 'no_course_chart' | 'no_config' | 'no_recipients' | 'send_failed' | null;
...
export class NotificationDispatcherService {
	...
	async dispatch(input: DispatchInput): Promise<DispatchResult> {
		...
		if (ctx === null) return { sent: false, reason: 'no_course_chart', recipients_count: 0, cc_count: 0 };

		const config = await this.loadConfig(ctx);
		if (config === null) return { sent: false, reason: 'no_config', recipients_count: 0, cc_count: 0 };

		const { toEmails, ccEmails, toStaffIds, ccStaffIds } = await this.resolveRecipients(ctx.course_chart_id, config);
		if (toEmails.length === 0) return { sent: false, reason: 'no_recipients', recipients_count: 0, cc_count: 0 };
		...
		try {
			...
			return { sent: true, recipients_count: toEmails.length, cc_count: ccEmails.length, reason: null };
		} catch (e) {
	...
	private async resolveContext(input: DispatchInput): Promise<ResolvedContext | null> {
		const rows = await this.dataSource.query(
			...
			FROM course_chart cc
			`,
			[input.chartId, input.periodId, input.triggerCode, TYPE_CODES.CHART_LEVEL_TYPE.COURSE_COORDINATOR, input.ifcStatusCode],
		);
	...
	private async buildSubstitutions(ctx: ResolvedContext, notifierUserId: number | null): Promise<Record<string, string>> {
		const paramRow = await this.dataSource.query(`SELECT value FROM core.parameters WHERE code = $1 LIMIT 1`, [IFCS_PARAMETER_KEYS.IFC_NOTIFICATION_VARS]);
		const vars: NotificationVar[] = paramRow[0]?.value ?? [];

		const statusCode = await this.lookupStatusCode(ctx.ifc_status_type_id);

		const subs: Record<string, string> = {};
		const allowed = (v: NotificationVar) => v.valid_status_codes === null || v.valid_status_codes.includes(statusCode);
		...
		for (const v of vars) {
			...
			switch (key) {
				...
				case '{{ifc_link}}':
					...
					break;
				case '{{observer_name}}':
					subs[key] = await this.lookupLatestStatusUserName(ctx.ifc_id, TYPE_CODES.IFC_STATUS.OBSERVED);
					break;
				case '{{comment}}':
					subs[key] = await this.lookupLatestStatusComment(ctx.ifc_id, TYPE_CODES.IFC_STATUS.OBSERVED);
					break;
				case '{{submitter_name}}':
					subs[key] = await this.lookupLatestStatusUserName(ctx.ifc_id, TYPE_CODES.IFC_STATUS.SUBMITTED);
					break;
				default:
					subs[key] = '';
			}
	...
	private async lookupStatusCode(statusTypeId: number): Promise<string> {
		const rows = await this.dataSource.query(`SELECT code FROM core.types WHERE id = $1 LIMIT 1`, [statusTypeId]);
		return rows[0]?.code ?? '';
	}

	private async lookupUserName(userId: number | null): Promise<string> {
		if (userId === null) return '';
		const rows = await this.dataSource.query(`SELECT first_name || ' ' || last_name AS name FROM organization.users WHERE id = $1 LIMIT 1`, [userId]);
		return rows[0]?.name ?? '';
	}

	private async lookupLatestStatusUserName(ifcId: number | null, statusCode: string): Promise<string> {
	...
	private async lookupLatestStatusComment(ifcId: number | null, statusCode: string): Promise<string> {
		if (ifcId === null) return '';
		const rows = await this.dataSource.query(
			`
	...
	private async writeLog(ctx: ResolvedContext, config: LoadedConfig, toStaffIds: number[], ccStaffIds: number[], notifierUserId: number | null, messageId: string) {
		await this.dataSource.query(
			...
			`,
			[ctx.ifc_id, ctx.course_chart_id, config.id, notifierUserId, JSON.stringify(toStaffIds), JSON.stringify(ccStaffIds), messageId],
		);
	}
}
