...
@Injectable()
export class ProjectsUploadRepository {
	...
	async callUploadFunction(
		...
		return await this.dataSource.query(
			'SELECT * FROM audit.fn_upload_projects($1::jsonb, $2, $3, $4)',
			[JSON.stringify(rows), academicPeriodId, userId, sourceFile],
		);
	}

	async callRollbackFunction(uploadLogId: number): Promise<void> {
		await this.dataSource.query('SELECT audit.fn_rollback_projects($1)', [uploadLogId]);
	}
}
