...
import {
	...
	articulationErrorMessages,
	articulationTemplateLabels,
} from '../model/articulation-template.labels';
...
@Injectable()
export class ArticulationUploadService {
	constructor(
		private readonly repository: ArticulationUploadRepository,
	...
	async processUpload(
		...
		const labels = articulationTemplateLabels[language];
		const messages = articulationErrorMessages[language];
	...
	async generateTemplate(lang: string): Promise<{ buffer: Buffer; fileName: string }> {
		...
		const labels = articulationTemplateLabels[language];
		const outcomeTypes = await this.repository.getOutcomeTypes(language);
		...
		for (const type of outcomeTypes) {
			legendSheet.addRow([type.code, type.name]);
		}
	...
	private resolveLanguage(lang?: string): string {
		return lang && articulationTemplateLabels[lang] ? lang : DEFAULT_TEMPLATE_LANGUAGE;
	}

	// Positional layout (header ignored):
	// outcomeCode | studyPlanCode | courseCode | outcomeTypeCode
	private parseWorkbook(workbook: ExcelJS.Workbook): ArticulationRow[] {
	...
	private async annotateErrors(
		...
		const worksheet = workbook.worksheets[0];
		// data columns = outcomeCode, studyPlanCode, courseCode, outcomeTypeCode; error column is next.
		const errorColumn = 5;
