...
@Injectable()
export class ArticulationUploadRepository {
	...
	async callUploadFunction(
		...
		return await this.dataSource.query(
			'SELECT * FROM audit.fn_upload_articulation($1::jsonb, $2, $3, $4)',
			[JSON.stringify(rows), academicPeriodId, userId, sourceFile],
		);
	}

	async callRollbackFunction(uploadLogId: number): Promise<void> {
		await this.dataSource.query('SELECT audit.fn_rollback_articulation($1)', [uploadLogId]);
	}

	async getOutcomeTypes(language: string): Promise<Array<{ code: string; name: string }>> {
		return await this.dataSource.query(
			`SELECT t.code, COALESCE(t.name->>$2, t.name->>'es', t.code) AS name
			 ...
			 ORDER BY t.code`,
			[TYPE_GROUP_CODES.OUTCOME_TYPE, language],
		);
	}
}
