...
@Injectable()
export class SectionsUploadRepository {
	...
	async callUploadFunction(
		...
		return await this.dataSource.query(
			'SELECT * FROM audit.fn_upload_sections($1::jsonb, $2, $3, $4)',
			[JSON.stringify(rows), academicPeriodId, userId, sourceFile],
		);
	}

	async callRollbackFunction(uploadLogId: number): Promise<void> {
		await this.dataSource.query('SELECT audit.fn_rollback_sections($1)', [uploadLogId]);
	}

	async getSectionModalities(language: string): Promise<Array<{ code: string; name: string }>> {
		return await this.dataSource.query(
