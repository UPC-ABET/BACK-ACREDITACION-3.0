...
jest.mock('../core/student-sections-upload.repository', () => ({ StudentSectionsUploadRepository: class {} }), {
	virtual: true,
});
...
describe('StudentSectionsUploadService — positional parsing', () => {
	it('sends structured section/student rows', async () => {
		...
		const result = await service.processUpload(buffer, 'studentsections.xlsx', 7, {
			academicPeriodId: 1,
		} as any);
	...
	it('returns annotated excel with localized text when the function reports row errors', async () => {
		...
		const result = await service.processUpload(buffer, 'studentsections.xlsx', 1, {
			academicPeriodId: 1,
			lang: 'es',
		} as any);
