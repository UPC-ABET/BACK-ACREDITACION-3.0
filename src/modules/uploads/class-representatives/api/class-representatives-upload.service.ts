...
import {
	...
	UploadResult,
	UploadRowError,
} from '../model/class-representatives.types';
import type { ClassRepresentativesUploadDto } from '../model/class-representatives.dtos';
import {
...
export class ClassRepresentativesUploadService {
	...
	async processUpload(
		...
		dto: ClassRepresentativesUploadDto,
	): Promise<UploadResult> {
		await this.uploadLogService.assertAcademicPeriodExists(academicPeriodId);

		const language = this.resolveLanguage(dto.lang);
		const labels = classRepresentativesTemplateLabels[language];
		const messages = classRepresentativesErrorMessages[language];
	...
	private resolveLanguage(lang?: string): string {
		return lang && classRepresentativesTemplateLabels[lang] ? lang : DEFAULT_TEMPLATE_LANGUAGE;
	}

	// Positional layout (header ignored): sectionCode | studentCode
	private parseWorkbook(workbook: ExcelJS.Workbook): ClassRepresentativeRow[] {
		const worksheet = workbook.worksheets[0];
		const rows: ClassRepresentativeRow[] = [];
