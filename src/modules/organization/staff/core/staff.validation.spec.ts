...
describe('StaffValidation', () => {
	...
	describe('validateUpdate', () => {
		it('passes when entity exists and no conflict', async () => {
			...
			mockRepo.findOneByCondition.mockResolvedValue(null);
			await expect(StaffValidation.validateUpdate(mockRepo as any, 1, {})).resolves.toBeUndefined();
		});
	...
	describe('validateDelete', () => {
		it('passes when entity exists', async () => {
			mockRepo.findOneById.mockResolvedValue({ id: 1 });
			await expect(StaffValidation.validateDelete(mockRepo as any, 1)).resolves.toBeUndefined();
		});
