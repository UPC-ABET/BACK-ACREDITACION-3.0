...
describe('ChartValidation', () => {
	...
	describe('validateCreate', () => {
		it('passes when no duplicate exists', async () => {
			...
			await expect(ChartValidation.validateCreate(mockRepo as any, { name: 'test' })).resolves.toBeUndefined();
		});

		it('throws when duplicate exists', async () => {
			mockRepo.findOneByCondition.mockResolvedValue({ id: 1 });
			await expect(ChartValidation.validateCreate(mockRepo as any, { name: 'test' })).rejects.toThrow(HttpException);
		});
	});
	...
	describe('validateUpdate', () => {
		...
		it('throws when entity not found', async () => {
			...
			mockRepo.findOneByCondition.mockResolvedValue(null);
			await expect(ChartValidation.validateUpdate(mockRepo as any, 999, {})).rejects.toThrow(HttpException);
		});
	});
	...
	describe('validateDelete', () => {
		...
		it('throws when entity not found', async () => {
			mockRepo.findOneById.mockResolvedValue(null);
			await expect(ChartValidation.validateDelete(mockRepo as any, 999)).rejects.toThrow(HttpException);
		});
	});
});
