...
describe('OrgScopeService', () => {
	...
	it('returns empty scope when SQL returns no rows', async () => {
		...
		expect(dataSource.query.mock.calls[0][1]).toEqual([1, 7, 5, TYPE_CODES.CHART_LEVEL_TYPE.SCHOOL_DIRECTOR]);
	});

	it('groups rows by level_num in ascending order and computes highest_level from anchors', async () => {
		dataSource.query.mockResolvedValueOnce([
			{ id: 30, parent_id: 20, level_num: 3, type_code: 'TG902-T003', label: { es: 'Programa' }, is_anchor: false },
			{ id: 20, parent_id: 10, level_num: 2, type_code: 'TG902-T002', label: { es: 'Escuela' }, is_anchor: true },
			{ id: 10, parent_id: null, level_num: 1, type_code: 'TG902-T001', label: { es: 'Decanato' }, is_anchor: false },
			{ id: 40, parent_id: 30, level_num: 4, type_code: 'TG902-T004', label: { es: 'Area' }, is_anchor: true },
		]);
	...
	it('coalesces multiple options per level into the same bucket', async () => {
		dataSource.query.mockResolvedValueOnce([
			{ id: 11, parent_id: 1, level_num: 2, type_code: 'TG902-T002', label: { es: 'A' }, is_anchor: true },
			{ id: 12, parent_id: 1, level_num: 2, type_code: 'TG902-T002', label: { es: 'B' }, is_anchor: false },
		]);
	...
	it('sets highest_level to null when no row is marked is_anchor', async () => {
		dataSource.query.mockResolvedValueOnce([{ id: 1, parent_id: null, level_num: 1, type_code: 'TG902-T001', label: {}, is_anchor: false }]);
