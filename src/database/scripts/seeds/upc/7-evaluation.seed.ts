...
runSeed('evaluation module', async (tenantDataSource) => {
	const projectValues = [
		[
			'PROJ_SOFT_FP_2026',
	...
	await tenantDataSource.query(`
		INSERT INTO "evaluation"."rubrics" (
			rubric_type_id,
			grade_type_id,
			evaluation_stage_type_id,
			study_plan_course_id
		)
		SELECT rubric_type.id, grade_type.id, evaluation_stage.id, spc.id
		FROM (
			VALUES
		...
		) AS v(rubric_type_code, grade_type_code, evaluation_stage_code, study_plan_code, academic_period_code, course_name)
		...
		JOIN "core"."types" evaluation_stage
			ON evaluation_stage.code = v.evaluation_stage_code
