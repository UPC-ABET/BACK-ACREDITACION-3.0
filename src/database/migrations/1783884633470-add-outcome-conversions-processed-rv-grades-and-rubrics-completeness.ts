...
/**
 * Combined migration:
 *  1. Creates `accreditation.outcome_conversions` and `academic.processed_rv_grades`.
 *  2. Widens `audit.fn_upload_rubrics`'s CAPSTONE + Multiple-competency-scope "complete
 *     commission" completeness check to also require CONTROL outcomes (previously only
 *     VERIFICATION outcomes were required per commission, silently ignoring CONTROL outcomes
 *     mapped to the same course). `v_is_capstone` detection stays VERIFICATION-only.
 */
export class AddOutcomeConversionsProcessedRvGradesAndRubricsCompleteness1783884633470 implements MigrationInterface {
	name = 'AddOutcomeConversionsProcessedRvGradesAndRubricsCompleteness1783884633470';

	public async up(queryRunner: QueryRunner): Promise<void> {
		// ── Part 1: outcome_conversions + processed_rv_grades tables ───────
		...
		// ── Part 2: rubrics completeness check widened to CONTROL outcomes ─
		await queryRunner.query(`
...
$fn$;
	...
	public async down(queryRunner: QueryRunner): Promise<void> {
		// ── Part 2 (reverse): restore prior fn_upload_rubrics (VERIFICATION-only completeness) ─
		await queryRunner.query(`
CREATE OR REPLACE FUNCTION audit.fn_upload_rubrics(
	p_rows               jsonb,
	p_academic_period_id integer,
...
$fn$;
		...
		// ── Part 1 (reverse): drop the new tables ─────────────────────────
