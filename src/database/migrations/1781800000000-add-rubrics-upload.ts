...
export class AddRubricsUpload1781800000000 implements MigrationInterface {
	public async up(queryRunner: QueryRunner): Promise<void> {
		...
		await queryRunner.query(
			`ALTER TABLE "evaluation"."rubrics" ADD COLUMN IF NOT EXISTS "upload_log_id" integer`,
		);
		await queryRunner.query(
			`ALTER TABLE "evaluation"."rubrics" ADD CONSTRAINT "FK_rubrics_upload_log_id"
			 FOREIGN KEY ("upload_log_id") REFERENCES "audit"."upload_logs"("id")
			 ON DELETE NO ACTION ON UPDATE NO ACTION`,
		);

		await queryRunner.query(
			`ALTER TABLE "evaluation"."rubric_questions" ADD COLUMN IF NOT EXISTS "upload_log_id" integer`,
		);
		await queryRunner.query(
			`ALTER TABLE "evaluation"."rubric_questions" ADD CONSTRAINT "FK_rubric_questions_upload_log_id"
			 FOREIGN KEY ("upload_log_id") REFERENCES "audit"."upload_logs"("id")
			 ON DELETE NO ACTION ON UPDATE NO ACTION`,
		);

		await queryRunner.query(
			`ALTER TABLE "evaluation"."rubric_question_criterias" ADD COLUMN IF NOT EXISTS "upload_log_id" integer`,
		);
		await queryRunner.query(
			`ALTER TABLE "evaluation"."rubric_question_criterias" ADD CONSTRAINT "FK_rubric_question_criterias_upload_log_id"
			 FOREIGN KEY ("upload_log_id") REFERENCES "audit"."upload_logs"("id")
			 ON DELETE NO ACTION ON UPDATE NO ACTION`,
		);
		...
		await queryRunner.query(`
			...
			SELECT tg.id, 'TG1101-T012',
				'{"es": "Rúbricas", "en": "Rubrics"}'::jsonb,
				'{}'::jsonb, true, NOW(), NOW()
...
BEGIN
	...
	LOOP
		...
		-- Alias 'rb' para evitar conflicto con variable de loop 'r'
		IF EXISTS (
...
$fn$;
		...
		// ── audit.fn_rollback_rubrics ─────────────────────────────────────
		await queryRunner.query(`
CREATE OR REPLACE FUNCTION audit.fn_rollback_rubrics(p_upload_log_id integer)
RETURNS void
LANGUAGE plpgsql
AS $fn$
BEGIN
	-- Block if rubric_scores exist linked to this upload
	IF EXISTS (
		SELECT 1
...
$fn$;
	...
	public async down(queryRunner: QueryRunner): Promise<void> {
		await queryRunner.query(`DROP FUNCTION IF EXISTS audit.fn_rollback_rubrics(integer)`);
		await queryRunner.query(
			`DROP FUNCTION IF EXISTS audit.fn_upload_rubrics(jsonb, integer, integer, text)`,
		);

		await queryRunner.query(`DELETE FROM core.types WHERE code = 'TG1101-T012'`);

		await queryRunner.query(
			`ALTER TABLE "evaluation"."rubric_question_criterias" DROP CONSTRAINT IF EXISTS "FK_rubric_question_criterias_upload_log_id"`,
		);
		await queryRunner.query(
			`ALTER TABLE "evaluation"."rubric_question_criterias" DROP COLUMN IF EXISTS "upload_log_id"`,
		);

		await queryRunner.query(
			`ALTER TABLE "evaluation"."rubric_questions" DROP CONSTRAINT IF EXISTS "FK_rubric_questions_upload_log_id"`,
		);
		await queryRunner.query(
			`ALTER TABLE "evaluation"."rubric_questions" DROP COLUMN IF EXISTS "upload_log_id"`,
		);

		await queryRunner.query(
			`ALTER TABLE "evaluation"."rubrics" DROP CONSTRAINT IF EXISTS "FK_rubrics_upload_log_id"`,
		);
		await queryRunner.query(
			`ALTER TABLE "evaluation"."rubrics" DROP COLUMN IF EXISTS "upload_log_id"`,
		);
	}
}
