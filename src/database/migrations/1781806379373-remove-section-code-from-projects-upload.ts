...
/*
 * Removes the sectionCode column from the projects bulk-upload Excel.
 * The system now auto-selects the first section (alphabetically by section_code)
 * in which the student is enrolled for the given course and academic period.
 *
 * Changes to audit.fn_upload_projects (v3):
 * - Phase 1: drops sectionCodeEmpty validation (no longer sent by the client).
 * - Phase 2: student enrollment check no longer filters by section_code.
 * - Phase 3: INSERT into project_students uses DISTINCT ON (st.code) ORDER BY cs.section_code ASC
 *            to pick the first section automatically.
 */
export class RemoveSectionCodeFromProjectsUpload1781806379373 implements MigrationInterface {
	name = 'RemoveSectionCodeFromProjectsUpload1781806379373';
                ...
                LOOP
                    ...
                    IF v_existing_id IS NULL THEN
                        ...
                        LOOP
                            ...
                            -- auto-detect enrollment: student must have at least one active enrollment
                            -- in the course during the academic period (section resolved automatically)
                            IF NOT EXISTS (
                                SELECT 1
	...
	public async down(queryRunner: QueryRunner): Promise<void> {
		// Restore v2: fn_upload_projects with sectionCode validation
		await queryRunner.query(`
            CREATE OR REPLACE FUNCTION audit.fn_upload_projects(
                p_rows               jsonb,
                p_academic_period_id integer,
